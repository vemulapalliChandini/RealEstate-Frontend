...
export default function MarketingAgentComplaints() {
  ...
  const filterBuyers = () => {
    ...
    const filtered = complaints.filter((b) => {
      const matchesName = b.name
        ? b.name.toLowerCase().includes(searchName.toLowerCase())
        : false; 
    
      const matchesLocation = b.district
        ? b.district.toLowerCase().includes(searchLocation.toLowerCase())
        : false; 
